...
function Notifier(tls_options, /* Bool */ development) {
  ...
  function notify(/* Notification */ notification, /* Function */ callback) {
    ...
    var _notification = new Notification(notification.device, notification.normalizedPayload());
    _notification.callback = callback;
  
    pending.push(_notification);
    process.nextTick(consume);
    return true;
  }
  ...
  */
  ...
  feedback(function(error) {
    if (error) {
      vlog("--- Startup Failed! An error occured during feedback query.");
      return;
    }
